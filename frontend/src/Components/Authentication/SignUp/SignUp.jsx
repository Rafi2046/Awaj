...
const SignUp = () => {
    ...
    const { user, createUser, emailVerification, uploadNameImageID } = useContext(AuthContext)
    ...
    const handleSubmit = async (e) => {
        ...
        createUser(formData.email, formData.password)
            .then(async (res) => {
                emailVerification();
                uploadNameImageID(formData.fName, formData.proPic)
                    .then(async (res) => {
                        try {
                            const response = await axios.post('http://localhost:8000/users.php', formData, {
                                headers: {
                            ...
                            <div className="flex gap-2">
                                {avatars.map((url, index) => (
                                    <img
                                        ...
                                        src={url}
                                        alt={`Avatar ${index + 1}`}
                                        className={`w-10 h-10 rounded-full cursor-pointer border-2 ${formData.proPic === url ? 'border-blue-500' : 'border-gray-300'
                                            }`}
